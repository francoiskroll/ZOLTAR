...
# would be easier for some users to input DATA.txt file generated from Jason's code
# so here is a small function to convert DATA.txt format to middur format generated by FramebyFrame
# then we can proceed as normal

DATAtoMid <- function(DATApath) {
  
  cat('\t \t \t \t >>> Importing data in DATA.txt format.\n')
  
  # import
  datxt <- read.table(DATApath, skip=2)
  
  # check it has expected number of columns
  if(ncol(datxt)!=99) stop('\t \t \t \t >>> File does not have 99 columns as expected. Something is wrong.\n')
  
  ## edits to make it like middur file
  
  # first two columns are useless but keep them so we turn them into zhrs & exsecs
  # bring CLOCK column (which is last) as second
  datxt <- datxt[, c(1, ncol(datxt), 2, 3:(ncol(datxt)-1))]
  colnames(datxt) <- c('fullts', 'zhrs', 'exsecs', sprintf('f%i', 1:96))
  datxt$fullts <- 0
  datxt$exsecs <- 0
  
  # CLOCK column (now zhrs) resets every day (like a clock!), it goes 0 >> 24, then 0 >> 24, etc.
  # zhrs is 0 at 9 AM day0, then keeps on increasing
  # to convert it, we could add 24 hrs to all values from first day, 48 hrs to all values from second day, etc.
  # but I can see differences between each row is always exactly 0.01666667 hr,
  # so it was probably created 'manually' by simply adding 1 minute for each row, rather than referring to timestamps recorded by ZebraLab
  # (Note: VpSorter.R in FramebyFrame does it slightly better, I think, because it uses timestamps given by ZebraLab, for each calculating number of hours since 9 AM)
  # here, we do not have access to the ZebraLab timestamps anyways so can do simpler solution which is to start with the first CLOCK value and iteratively add 1 minute
  datxt$zhrs <- seq(from=datxt$zhrs[1], by=1/60, length.out=nrow(datxt)) # 1/60 is 1 min in hours
  
  # it is now in middur format, return
  return(datxt)
}
